...
def index(request):
    date_now = date.today()
    auction_property = Properti.objects.filter(
        status="Auction", bidding_end_time=date_now)
    property = []
    expired_property = []
...
def rent(request):
    ...
    if sort_by and not sort_by == 'none_':
        ...
        # price__gte, price__lte
    content = {'rent': rent_properti, 'sort_by': sort_by,
...
def auction(request):
    auction_property = Properti.objects.filter(
        status='Auction', is_approved=True)
    ...
    if request.method == 'POST':
        ...
        if value and agent and not value == 'none':
            if AgentRating.objects.filter(rating_by=request.user, agent_id=agent).exists():
                AgentRating.objects.filter(
                    rating_by=request.user, agent_id=agent).update(value=value)
            else:
                AgentRating.objects.create(
                    agent_id=agent, value=value, rating_by=request.user)
...
def bid(request, pk):
    ...
    user = request.user
    try:
        user_bid = Bidders.objects.get(
            properti=auction_property, user=user).bid_amount
    except:
        user_bid = 0
    if request.method == 'GET':
    ...
    if request.method == 'POST':
        ...
        if Bidders.objects.filter(user=request.user, properti=auction_property).exists():
            Bidders.objects.filter(
                user=request.user, properti=auction_property).update(bid_amount=bidamt)
        else:
            Bidders.objects.create(
                properti=auction_property, user=user, bid_amount=bidamt)
        try:
            user_bid = Bidders.objects.get(
                properti=auction_property, user=user).bid_amount
        except:
